...
describe("UrlService", () => {
  ...
  describe("createUrl", () => {
    ...
    it("should throw an error if customShortId is already in use", async () => {
      ...
      await expect(
        UrlService.createUrl(longUrl, customShortId)
      ).rejects.toThrow("Custom short ID already in use");
    });
  });
  ...
  describe("getLongUrl", () => {
    ...
    it("should throw an error for an invalid shortId", async () => {
      await expect(
        UrlService.getUrlByShortId("invalidShortId")
  ...
  describe("updateUrl", () => {
    ...
    it("should throw an error if the URL does not exist", async () => {
      await expect(
        UrlService.updateUrl("invalidShortId", {
  ...
  describe("deleteUrl", () => {
    ...
    it("should throw an error if the URL does not exist", async () => {
      await expect(UrlService.deleteUrl("invalidShortId")).rejects.toThrow(
        "URL not found"
      );
    });
  });
